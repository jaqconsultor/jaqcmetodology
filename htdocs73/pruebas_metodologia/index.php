...
$cDSN = "sqlsrv:Server=DESKTOP-3RUM6QP\SQLEXPRESS,1433;Database=trodu";
$cUsuario = "sa";
$cClave = "password2017";

$c = new PDO($cDSN, $cUsuario, $cClave);

if ($c) {
	echo "Coneccion Satisfactoria a SQL Server <br><br>";
} else {
	die( "Error: Conectando a la Base de Datos de SQL Server " + $cDSN);
}

// Hacer una Sentencia Select en una Tabla
$codigo = 1;
$sth = $c->prepare('SELECT * FROM aereolineas WHERE id = :codigo ');
$sth->execute(array(':codigo' => $codigo));
echo "Cantidad de Registros " . $sth->rowCount();
echo "<br>";
echo "<br>";
...
print_r($result);
echo "<br>";
echo "Impresion del Texto " . $result[0]['nb_aereolinea'];
echo "<br>";
die("prueba del pdo");
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        /*
         * $estructuracc = $estructurac . $tabla . '/';
         * if (!file_exists($estructuracc)) {
         * if(!mkdir($estructuracc, 0777, true)) {
         * //die('Fallo al crear las carpetas..1.');
         * }
         * }
         */

        $estructura0 = '' . $resultado1 . '/';
        if (! file_exists($estructura0)) {
            if (! mkdir($estructura0, 0777, true)) {
                // die('Fallo al crear las carpetas..1.');
            }
        ...
        $filec = $estructura0 . "c_" . $tabla . ".php";

        $filep = $estructura0 . "v_" . $tabla . ".php";
        $filel = $estructura0 . "lista.php";
        $filef = $estructura0 . "forma.php";
        $fileb = $estructura0 . "busqueda.php";

        //$nfilec = fopen($filec, 'w');
        $nfilep = fopen($filep, 'w');

        // $nfilel = fopen($filel, 'w');
        // $nfilef = fopen($filef, 'w');
        // $nfileb = fopen($fileb, 'w');

        $rfilec = fopen('Controlador.php', 'r');
        $rfilep = fopen('vista.php', 'r');
        // $rfilel = fopen('lista.php', 'r');
        // $rfilef = fopen('forma.php', 'r');
        // $rfileb = fopen('buscar.php', 'r');
